...
2. `CALL` stores only the return `pc`.
3. The callee creates its return slot on entry, above the incoming arguments,
   and treats arguments as locals at fixed offsets from the current stack
...
4. On `return expr`, the callee evaluates `expr`, stores it into its return
   slot with `SETLN`, pops the rest of its live frame/arguments with
